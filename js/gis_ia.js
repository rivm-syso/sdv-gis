...
function gis_ia_get_layer_div(map_id) {
	...
	for (t=0;t<GIS_ia_maps[map_id].layers_def.length;t++) {
		...
		if (extra_info) {
			...
			if (GIS_ia_maps[map_id].l.substr(3,1)!=='0') {r+='<div><div>Download</div>Download CSV data: <button class="gis_ia_filters_button" onclick="startDownload('+map_id+','+t+',false);">Download'+(GIS_ia_maps[map_id].l.substr(4,1)==='1'?' NL':'')+(GIS_ia_maps[map_id].l.substr(4,1)==='1'?' <button onclick="startDownload('+map_id+','+t+',true);">Download BB</button>':'')+'</div>';}
