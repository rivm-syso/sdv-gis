...
function gis_ia_filters_opa(map_id, no, d) {
	var el = jQuery('#gis_ia_filters_opa_'+map_id+'_'+no), v=parseFloat(el.attr('opacity'))+d;
	if (v>=0 && v<=1) {
...
function gis_ia_get_layer_div(map_id) {
	...
	for (t=0;t<GIS_ia_maps[map_id].layers_def.length;t++) {
		...
		if (extra_info) {
			...
			if (GIS_ia_maps[map_id].l.substr(2,1)==='1') {r+='<div><div>Transparantie</div><div class="button-opa" id="gis_ia_filters_opa_'+map_id+'_'+t+'" opacity="'+l.opacity+'">Transparantie: '+(l.opacity*100)+'%</div><div title="Minder transparantie" class="button-opa-min" onclick="gis_ia_filters_opa('+map_id+','+t+',-0.1);"></div><div title="Meer transparantie" class="button-opa-max onclick="gis_ia_filters_opa('+map_id+','+t+',0.1);""></div></div>';}
