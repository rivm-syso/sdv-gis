...
gis_ia_filter.prototype.html=function(depth) {
	...
	switch (this.t) {
		...
		case 'vt': // van - tot
			...
			if (this.l1=='') {
				r+='<div class="gis_ia_input gis_ia_input50"><label class="gis_ia_input" id="'+this.ID()+'-van-label" onclick="jQuery(this).hide();jQuery(\'#'+this.ID()+'-van\').focus();">'+this.v1+'</label><div><input class="gis_ia_input" id="'+this.ID()+'-van" type="number" min="'+this.mi1+'" max="'+this.ma1+'" step="'+this.st1+'" onchange="gis_ia_filters.change('+this.map_id+',\''+this.ID()+'\',0);" onkeydown="gis_ia_filters.keydown('+this.map_id+',\''+this.ID()+'\',0);"><span>'+this.x_button(2)+'</span></div></div>';
			} else {
				...
				r+='<div class="gis_ia_input gis_ia_input50"><label class="gis_ia_input" id="'+this.ID()+'-van-label" onclick="jQuery(this).hide();jQuery(\'#'+this.ID()+'-van\').focus();">'+this.v1+'</label><div><select id="'+this.ID()+'-van" class="gis_ia_input" onchange="gis_ia_filters.change('+this.map_id+',\''+this.ID()+'\',0);">'+opts+'</select><span>'+this.x_button(2)+'</span></div></div>';
			}
			if (this.l2=='') {
				r+='<div class="gis_ia_input gis_ia_input50"><label class="gis_ia_input" id="'+this.ID()+'-tot-label" onclick="jQuery(this).hide();jQuery(\'#'+this.ID()+'-tot\').focus();">'+this.v2+'</label><div><input class="gis_ia_input" id="'+this.ID()+'-tot" type="number" min="'+this.mi2+'" max="'+this.ma2+'" step="'+this.st2+'" onchange="gis_ia_filters.change('+this.map_id+',\''+this.ID()+'\',1);" onkeydown="gis_ia_filters.keydown('+this.map_id+',\''+this.ID()+'\',1);"><span>'+this.x_button(2)+'</span></div></div>';
			} else {
				var opts='<option></option>', os=this.l2, ot; os=os.replace(/\r\n/g,String.fromCharCode(13)); os=os.replace(/\n/g,String.fromCharCode(13)); os=os.split(String.fromCharCode(13)); for (ot=0;ot<os.length;ot++) {opts+='<option>'+os[ot]+'</option>';}
				r+='<div class="gis_ia_input gis_ia_input50"><label class="gis_ia_input" id="'+this.ID()+'-tot-label" onclick="jQuery(this).hide();jQuery(\'#'+this.ID()+'-tot\').focus();">'+this.v2+'</label><div><span class="gis_ia_filters_arrow_down"></span><select id="'+this.ID()+'-tot" class="gis_ia_input" onchange="gis_ia_filters.change('+this.map_id+',\''+this.ID()+'\',1);">'+opts+'</select><span>'+this.x_button(2)+'</span></div></div>';
			}
			r+='</div>';
			break;
	}
