...
(function (root, factory) {
  if(typeof define === "function" && define.amd) {
    ...
    ol.control.Filter = function(opt_options) {
		...
		jQuery('.gis_ia_filters_submenu').click(function(e) {e.preventDefault();});
    };
...
function gis_ia_get_layer_div(map_id) {
	...
	for (t=0;t<GIS_ia_maps[map_id].layers_def.length;t++) {
		...
		if (extra_info) {
			...
			if (GIS_ia_maps[map_id].l.substr(2,1)==='1') {r+='<div><div>Transparantie</div>Transparantie: <div class="button-opa" id="gis_ia_filters_opa'+map_id+'_'+t+'" opacity="'+l.opacity+'">'+(l.opacity*100)+'%</div><div title="Minder transparantie" class="button-opa-min"></div><div title="Meer transparantie" class="button-opa-max"></div></div>';}
