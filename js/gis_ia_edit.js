...
function getFilterDefTableItem(f,i,opts) {
	...
	switch (f.t) {
		...
		case 'd': // list
			...
			s+='<td><div style="'+(f.s=='0'?'':'display: none;')+'">Placeholder: <input value="'+f.p+'"></div><br>'+gis_ia_getSelect(['0=Selectbox','1=Radio buttons','3=Checkboxes'],f.s,'if (jQuery(this).val()==\'0\') {jQuery(jQuery(jQuery(this).parent()).children()[0]).show();} else {jQuery(jQuery(jQuery(this).parent()).children()[0]).hide();} gis_ia_set_filterItem(this,\''+i+'\',\'p\'); gis_ia_set_filterItem(this,\''+i+'\',\'s\');');
