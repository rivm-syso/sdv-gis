...
function getFilterDefTableItem(f,i,opts) {
	...
	switch (f.t) {
		case 'a': // header
			s+='<td>Tekst</td><td><input value="'+f.v+'" onchange="gis_ia_set_filterItem(this,\''+i+'\',\'v\');"></td><td style="'+inspringen+'"></td><td></td><td>'+gis_ia_getSelect(['t=Tekst','h2=Header 2','h3=Header 3'],f.s,'gis_ia_set_filterItem(this,\''+i+'\',\'s\');')+'</td><td><input onclick="gis_ia_del_filter(\''+i+'\');" type="button" value="Verwijder" class="button js-form-submit form-submit"></td></tr>';
			break;
		case 'g': // groep
			s+='<td>Groep</td><td><input value="'+f.w+'" onchange="gis_ia_set_filterItem(this,\''+i+'\',\'w\');"></td><td style="'+inspringen+'"></td><td>Soort:&nbsp;&nbsp;&nbsp;&nbsp;'+gis_ia_getSelect(['of=Or','en=And'],f.s,'gis_ia_set_filterItem(this,\''+i+'\',\'s\');')+'</td><td>'+gis_ia_getSelect(['0=In place','1=As submenu'],f.b,'gis_ia_set_filterItem(this,\''+i+'\',\'b\');',((''+i).indexOf('.')>=1?'style="display: none;"':''))+'</td><td><input onclick="gis_ia_del_filter(\''+i+'\');" type="button" value="Verwijder" class="button js-form-submit form-submit"></td></tr>';
		...
		case 'c': // checkbox
			...
			s+='</td><td>Waarde: <input value="'+f.w+'" onchange="gis_ia_set_filterItem(this,\''+i+'\',\'w\');" size="6"></td><td style="white-space: nowrap;"><input type="checkbox" '+(f.x0=='1'?'checked="checked" ':'')+'onchange="gis_ia_set_filterItem(this,\''+i+'\',\'x0\');" id="gis_ia_f_'+i+'x0"><label for="gis_ia_f_'+i+'x0"> X-button bovenaan</label><br><input type="checkbox"'+(f.x1=='1'?'checked="checked" ':'')+'onchange="gis_ia_set_filterItem(this,\''+i+'\',\'x1\');" id="gis_ia_f_'+i+'x1"><label for="gis_ia_f_'+i+'x1"> X-button in hoofdgroep</label><br><input type="checkbox"'+(f.x2=='1'?'checked="checked" ':'')+'onchange="gis_ia_set_filterItem(this,\''+i+'\',\'x2\');" id="gis_ia_f_'+i+'x2"><label for="gis_ia_f_'+i+'x2"> X-button in place</label>';
		...
		case 'd': // list
			...
			s+='<td>'+gis_ia_getSelect(['0=Selectbox','1=Radio buttons','3=Checkboxes'],f.s,'if (jQuery(this).val()==\'0\') {jQuery(jQuery(jQuery(this).parent()).children()[1]).show();} else {jQuery(jQuery(jQuery(this).parent()).children()[1]).hide();} gis_ia_set_filterItem(this,\''+i+'\',\'p\'); gis_ia_set_filterItem(this,\''+i+'\',\'s\');');
			s+='<div style="'+(f.s=='0'?'':'display: none;')+'">Placeholder: <input value="'+f.p+'"></div>';
			s+='<div><input type="checkbox" '+(f.x0=='1'?'checked="checked" ':'')+'onchange="gis_ia_set_filterItem(this,\''+i+'\',\'x0\');" id="gis_ia_f_'+i+'x0"><label for="gis_ia_f_'+i+'x0"> X-button bovenaan</label><br><input type="checkbox"'+(f.x1=='1'?'checked="checked" ':'')+'onchange="gis_ia_set_filterItem(this,\''+i+'\',\'x1\');" id="gis_ia_f_'+i+'x1"><label for="gis_ia_f_'+i+'x1"> X-button in hoofdgroep</label><br><input type="checkbox"'+(f.x2=='1'?'checked="checked" ':'')+'onchange="gis_ia_set_filterItem(this,\''+i+'\',\'x2\');" id="gis_ia_f_'+i+'x2"><label for="gis_ia_f_'+i+'x2"> X-button in place</label></div>';
			s+='</td><td><input onclick="gis_ia_del_filter(\''+i+'\');" type="button" value="Verwijder" class="button js-form-submit form-submit"></td></tr>';
			break;
		case 'vt': // van - tot
			var a=[], t, t1; for (t=0;t<gis_ia_fieldsOnServer.length;t++) {for (t1=0;t1<gis_ia_fieldsOnServer[t].length;t1++) {a[a.length]=t+'.'+gis_ia_fieldsOnServer[t][t1]+'='+(t+1)+' '+gis_ia_fieldsOnServer[t][t1];}}
			...
			s+='<div><b>Tot</b></div>';
			s+='<div style="margin-bottom: 12px;"><div style="display: inline-block; width: 140px;">Achtergrond tekst:</div><input style="width: 100px;" value="'+f.v2+'" onchange="gis_ia_set_filterItem(this,\''+i+'\',\'v2\');"></div>';
...
function redrawFilterDefsTable() {
    ...
    var tbl='<table class="field-multiple-table responsive-enabled"><thead><tr><th>Pos.</th><th>Type</th><th>Label(s)</th><th>Veld / Operator</th><th>Definitie</th><th>Weergave</th><th></th></tr></thead><tbody>';
	for (t=0,t1=fs.length;t<t1;t++) {
