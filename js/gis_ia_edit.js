...
function getFilterDefTableItem(f,i,opts) {
	...
	switch (f.t) {
		...
		case 'd': // list
			...
			s+='<td><div id="gis_ia_list_'+this.ID()+'" style="'+(f.s=='0'?'':'display: none;')+'">Placeholder: <input value="'+f.p+'" onchange="if (jQuery(this).val()==\'0\') {jQuery(\'#gis_ia_list_'+this.ID()+'\').show();} else {jQuery(\'#gis_ia_list_'+this.ID()+'\').hide();} gis_ia_set_filterItem(this,\''+i+'\',\'p\');"></div><br>'+gis_ia_getSelect(['0=Selectbox','1=Radio buttons','3=Checkboxes'],f.s,'gis_ia_set_filterItem(this,\''+i+'\',\'s\');');
